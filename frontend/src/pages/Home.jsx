...
import AqiCard from "../components/dashboard/Home/AqiCard";
import AQIForecastChart from "../components/dashboard/AQIForecastChart";
...
const HomePage = () => {
  ...
  const fetchHistoricalData = async (zone, dateFilter) => {
    try {
      ...
      let endpoint = `${import.meta.env.VITE_BACKEND_URL}/api/aggregated`;
  ...
  useEffect(() => {
    ...
    const fetchAqiData = async () => {
      ...
      try {
        const currentResponse = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/data/latest`);
